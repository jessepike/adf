...
| B59 | Review function tuning sprint — run 10-20 reviews, log observations in KB, tune prompts/config. Tracking: `kb/EXTERNAL-REVIEW-MODEL-RELIABILITY.md` | Enhancement | Review | P1 | M | Develop | in_progress |
...
| B90 | **Health check doesn't recognize `discover-brief.md`** — `check_project_health` hardcodes `docs/brief.md` but ADF convention renames it to `docs/discover-brief.md` at Discover completion. Health check should accept both names, or look for `docs/*-brief.md`. Discovered during Nerve Center Discover finalization. | Bug | adf-server | P2 | S | — | pending |
| B91 | **Clarify brief rename convention (`brief.md` → `discover-brief.md`)** — ADF-DISCOVER-SPEC references `discover-brief.md` as the output, but the brief spec and health check expect `brief.md`. The rename adds friction without clear value — the brief is already tagged with stage in frontmatter. Decide: (a) keep rename convention and fix tooling, (b) drop rename and keep `brief.md` throughout, or (c) make rename optional. | Spec gap | specs | P2 | S | — | pending |
