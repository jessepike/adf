...
export function registerOrchestrationTools(server) {
    ...
    server.tool("get_review_prompt", "Get the review prompt for a stage and review phase. Use when preparing to run an internal (Ralph Loop) or external review. Returns the full prompt text ready for use.", {
    ...
    server.tool("get_transition_prompt", "Get guidance for transitioning between ADF stages. Returns the transition prompt content. Set validate=true to also check prerequisites against the project's current state (reads status.md and brief).", {
        ...
        if (!promptCheck.valid)
            return errorResponse(promptCheck.error);
        ...
        if (!validate) {
            return { content: [{ type: "text", text: promptContent }] };
        }
        ...
        if (!statusCheck.valid)
            return errorResponse(statusCheck.error);
        ...
        if (briefCheck.valid && await fileExists(briefCheck.resolved)) {
            ...
            if (!matchCheck.valid)
                return errorResponse(matchCheck.error);
        ...
        if (transition === "discover_to_design") {
            if (statusData.stage !== "discover")
                issues.push(`Expected stage 'discover', got '${statusData.stage}'`);
        }
        else if (transition === "design_to_develop") {
            if (statusData.stage !== "design")
                issues.push(`Expected stage 'design', got '${statusData.stage}'`);
        }
        else if (transition === "develop_to_deliver") {
            if (statusData.stage !== "develop")
                issues.push(`Expected stage 'develop', got '${statusData.stage}'`);
        }
        if (issues.length === 0) {
