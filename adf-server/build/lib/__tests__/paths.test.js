...
describe("validatePathWithinBase", () => {
    it("accepts paths within base", async () => {
        const result = await validatePathWithinBase(path.join(ADF_ROOT, "kb/README.md"), ADF_ROOT);
        expect(result.valid).toBe(true);
    });
    it("rejects paths escaping base with ..", async () => {
        const result = await validatePathWithinBase(path.join(ADF_ROOT, "../../../etc/passwd"), ADF_ROOT);
    ...
    it("handles non-existent paths gracefully", async () => {
        const result = await validatePathWithinBase(path.join(ADF_ROOT, "nonexistent-file.md"), ADF_ROOT);
        expect(result.valid).toBe(true);
    });
